...
(function() {
  var Arr, Break, Command, CommandList, CommandState, Compass, CompassEvent, CompassUtil, Easing, Err, Event, EventDispatcher, Func, JqueryAjax, JqueryGet, JqueryPost, Listen, Namespace, Num, ParallelList, Return, Scene, SerialList, Tween, Wait, _NamespaceObject,
    __slice = [].slice,
  ...
  */
  ...
  Compass = (function(_super) {
    ...
    Compass.VERSION = '0.0.1';

    function Compass(option) {
      this._kazitoriChangeHandler = __bind(this._kazitoriChangeHandler, this);
      ...
      option.routes = (_ref = option.route) != null ? _ref : this.routes;
      option.root = (_ref1 = option.root) != null ? _ref1 : this.root;
      this._kazitori = new Kazitori(option);
      this._kazitori.addEventListener(KazitoriEvent.CHANGE, this._kazitoriChangeHandler);
      this._log = CompassUtil.log;
      this._isReleaseMode = (_ref2 = option.isReleaseMode) != null ? _ref2 : true;
      CompassUtil.LOG_ENABLED = true;
      CompassUtil.printInit(this._kazitori);
    }

    Compass.prototype._kazitoriChangeHandler = function(event) {
      var flow, next, prev;
      prev = event.prev;
      next = event.next;
      this._log("[Compass] change : prev = '" + prev + "' next = '" + next + "'");
      this.dispatchEvent(CompassEvent.CHANGE, {
        prev: prev,
        next: next
      });
      flow = CompassUtil.calcPath(prev, next);
      return this._log("[Compass] change :  '" + (flow.join('\' -> \'')) + "'");
    };

    Compass.prototype._startAsync = function() {};

    Compass.prototype.goto = function(path) {
      this._log("[Compass] goto : '" + path + "'");
      return this._kazitori.change(path);
    };

    Compass.prototype.next = function() {
      this._log('[Compass] next');
      return this._kazitori.torikazi();
    };

    Compass.prototype.prev = function() {
      this._log('[Compass] prev');
  ...
  */
  ...
  CompassUtil = (function() {
    ...
    CompassUtil.calcPath = function(prev, next) {
      var branch, flow, i, n, nexts, prevs, s, sub, _i, _j, _len, _len1;
      if (prev === null) {
        prev = '';
      }
      if (next === null) {
      ...
      s = '';
      i = 0;
      for (_i = 0, _len = prevs.length; _i < _len; _i++) {
        sub = prevs[_i];
        s += '/' + sub;
        prevs[i] = s;
        ++i;
      ...
      s = '';
      i = 0;
      for (_j = 0, _len1 = nexts.length; _j < _len1; _j++) {
        sub = nexts[_j];
        s += '/' + sub;
        nexts[i] = s;
        ++i;
    ...
    CompassUtil.log = function() {
      ...
      if (CompassUtil.LOG_ENABLED) {
        return console.log(m.join(' '));
      }
    };

    CompassUtil.printInit = function(kazitori) {
      CompassUtil.log("----------------------------------------");
  ...
  Scene = (function() {
    ...
    */
    ...
    function Scene(path) {
      ...
      this._level = this._paths.length - 1;
      this._isRoot = this._level === 0;
      this._parent = this._isRoot ? null : this._paths[this._level - 1];
    }
    ...
    */
    ...
    Scene.prototype.getPaths = function() {
      return this._paths;
    };
    ...
    Scene.prototype.getIsRoot = function() {
      return this._isRoot;
    };
