...
var command:JPP.Serial = new JPP.Serial(
	new JPP.Func(function() {
		trace("Start");
	}),
	new JPP.Wait(0.5),
	new JPP.Func(function() {
	...
	new JPP.JqueryGet('sample.xml', null, function(data:XMLHttpRequest) {
		trace(data);
	}),
	new JPP.Func(function() { trace("Load Complete"); }),
	//function() { trace("Load Complete (js function)"); },

	new JPP.Tween(self, { value0 : 100, value1 : 0 }, { value0 : 0 }, 1, JPP.Easing.easeOutExpo,
		function() {
	...
	new JPP.Serial(
		...
		new JPP.Parallel(
			new JPP.Serial(
				new JPP.Wait(1),
				new JPP.Func(function() { trace("C"); }),
				new JPP.Break(),
				new JPP.Func(function() { trace("Never"); })
			),
			new JPP.Serial(
				new JPP.Wait(0.5),
				new JPP.Func(function() {
					...
					this.getParent().insertCommand(
						new JPP.Wait(2),
						new JPP.Func(function() { trace("E"); })
					);
				}),
				new JPP.Func(function() { trace("F"); })
			),
			new JPP.Serial(
				new JPP.Wait(1.5),
				new JPP.Func(function() { trace("D"); })
			)
		),
		new JPP.Func(function() { trace("G"); })
	)
);
command.execute();